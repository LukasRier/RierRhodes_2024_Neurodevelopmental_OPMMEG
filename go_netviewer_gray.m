function go_netviewer_gray(C)
% only use to show full networks w matrtices of 1s and zeros
cLims = [0,1];
...
sphereWidths = 3*sphereWidths./max(sphereWidths(:));
...
maxweight = 1.5;
emap = (linspace(-maxweight,maxweight,length(cmap))).^2;
...
for ii = 1:length(aalviewer.centroids);
    [x y z] = sphere(10);
    sw = sphereWidths(ii);
    surf(sw*x+aalviewer.centroids(ii,1),sw*y+aalviewer.centroids(ii,2),sw*z+aalviewer.centroids(ii,3),'edgecolor','none','facecolor',[1,2,1].*0.2,'facealpha',1);
end
