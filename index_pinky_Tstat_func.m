function index_pinky_Tstat_func(sub,ses,project_dir)
restoredefaultpath
cleaning_only = 0;
fake_bad_ch =1;
...
if ~exist(VOI_mat_file,'file')
    ...
    save(VOI_mat_file,'sourcepos','sourcepos_vox')
else
    load(VOI_mat_file,'sourcepos','sourcepos_vox')
end
...
%     lp = 25;   
    ...
    %% Beamform
    % Index
    pseudoT_D1 = get_pseudoT(C_data_f_Digit1,Ca_Digit1,Cc_Digit1,Lead_fields);
    %
    ...
    %%
    [mean_Env_index] = get_envelope(Digit1_trials,C_data_f_Digit1,Lead_fields,min_voxel_D1,control_inds);
    [mean_Env_index_pinkytrls] = get_envelope(Digit4_trials,C_data_f_Digit4,Lead_fields,min_voxel_D1,control_inds);
    figure
    set(gcf,'Color','w');
    ...
    %% Pinky
    pseudoT_D4 = get_pseudoT(C_data_f_Digit4,Ca_Digit4,Cc_Digit4,Lead_fields);
    %
    ...
    % get peak voxel timecourse
    %%
    [mean_Env_pinky] = get_envelope(Digit4_trials,C_data_f_Digit4,Lead_fields,min_voxel_D4,control_inds);
    [mean_Env_pinky_indextrls] = get_envelope(Digit1_trials,C_data_f_Digit1,Lead_fields,min_voxel_D4,control_inds);
    figure
    set(gcf,'Color','w');
...
%     ax.Children.HandleVisibility = 'off';
    ...
    save([path_Tstat,'D1_peak.mat'],'dip_loc_D1')
    save([path_Tstat,'D4_peak.mat'],'dip_loc_D4')
    %% save Tstat maps
    image = ft_read_mri([path_mri,files_mri]); 
